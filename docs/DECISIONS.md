...
## 2026-07-21 — Wave-2 prefetch: profile generation starts when tracks are requested
Gabe's directive, extending the parallelize-within-waves rule: identity + portrait + cover now start in the background the moment the user requests candidate tracks, so the profile step is usually instant after the pick. This supersedes the earlier "wave boundary is sequential" note — that was about the *pick* being real (tracks), and wave 2 never depended on the pick. The prefetch is keyed by DNA object identity (any edit produces a new object → stale prefetch misses and pickSingle regenerates fresh); failures resolve to null and fall back to on-demand generation, never surfacing an error while the user is still listening. Client-side only — no API changes.
